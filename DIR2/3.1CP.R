...
# Load datasets
centri_gamma  <- read_csv("1centri_gammaAS.csv")
eventi_verita <- read_csv("1eventi_veritaAS.csv")
gamma_prob    <- read_csv("1gamma_probAS.csv")
stazioni      <- read_csv("1stazioni_networkAS.csv")
picks         <- read_csv("1picks_simulatiAS.csv")
...
plot_errori_ipocentro <- function(id_reale) {
  ...
  # Set view range dynamically (minimum 2km range or twice the error distance)
  ...
  p_xy <- ggplot(df_c, aes(X, Y, color = Type, shape = Type)) +
    ...
    labs(title = "XY View (Map)", x = "X (km)", y = "Y (km)", color = "Type", shape = "Type") +
    thesis_theme
  ...
  p_xz <- ggplot(df_c, aes(X, Z, color = Type, shape = Type)) +
    ...
    labs(title = "XZ View (Profile)", x = "X (km)", y = "Z (km)", color = "Type", shape = "Type") +
    thesis_theme
  ...
  p_yz <- ggplot(df_c, aes(Y, Z, color = Type, shape = Type)) +
    ...
    labs(title = "YZ View (Profile)", x = "Y (km)", y = "Z (km)", color = "Type", shape = "Type") +
    thesis_theme
  ...
  final_plot + plot_annotation(
    ...
    subtitle = paste("3D Euclidean Distance (Error):", round(dist_km, 3), "km | Grid: 1 tick = 1 km"),
...
# ==============================================================================
# CONFORMAL PREDICTION SETS FOR TIME
# ==============================================================================
...
# Display mean residuals per cluster
df_lavoro %>% 
  group_by(cluster_id) %>% 
  summarise(mean_residual = mean(residuo))

# Join assignment probabilities
df_lavoro <- df_lavoro %>%
...
# 3. CONFORMAL CALIBRATION & VALIDATION LOOP
alpha <- 0.05
risultati_conformal <- list()

for (i in 1:3) {
  # (i=1 -> cluster 0, i=2 -> cluster 1, i=3 -> cluster 2)
  ...
  valid_set <- cluster_info$valid %>%
    ...
    mutate(
      ...
      t_min = timestamp - margine,
      t_max = timestamp + margine,
  ...
  cat(paste0("CLUSTER ", cl_id, " Conformal Results:\n"))
  cat(paste0("  > q_hat (Quantile ", (1-alpha)*100, "%): ", round(q_hat, 4), "\n"))
  cat(paste0("  > Coverage: ", round(copertura_empirica, 2), "%\n"))
  cat(paste0("  > Example (first pick): [", 
             round(valid_set$t_min[1], 3), " , ", 
             round(valid_set$t_max[1], 3), "] : Observed Target = ", 
             valid_set$timestamp[1], "\n\n"))
}


# ==============================================================================
# SPATIAL INVERSION & JACOBIAN REGIONS
# ==============================================================================
...
for (i in 0:2) {
  ...
  C_spaziale[3,3] <- C_spaziale[3,3] * 2 * vp_vs_ratio
  ...
  cat("CLUSTER", i, "Inversion completed with Vp/Vs scaling factor.\n")
...
# ==============================================================================
# CONFORMAL CONFIDENCE REGION PLOTS
# ==============================================================================
...
# Run Conformal Region Plots
plot_ev0 <- plot_conformal_ipocentro(0)
plot_ev1 <- plot_conformal_ipocentro(1)
plot_ev2 <- plot_conformal_ipocentro(2)


# ==============================================================================
# SAVING PLOTS FOR THESIS MANUSCRIPT
# ==============================================================================

# Standard sizing parameters for thesis template grid (optimal for side-by-side)
thesis_width  <- 10
thesis_height <- 4.5

# 1. Export First Set (Localization Error Analysis)
ggsave("3uno.pdf", plot = plot_errori_ipocentro(0), width = thesis_width, height = thesis_height, dpi = 300)
ggsave("3due.pdf", plot = plot_errori_ipocentro(1), width = thesis_width, height = thesis_height, dpi = 300)
ggsave("3tre.pdf", plot = plot_errori_ipocentro(2), width = thesis_width, height = thesis_height, dpi = 300)

# 2. Export Second Set (Conformal Confidence Regions)
ggsave("fig_2.1.1.pdf", plot = plot_ev0, width = thesis_width, height = thesis_height, dpi = 300)
ggsave("fig_2.1.2.pdf", plot = plot_ev1, width = thesis_width, height = thesis_height, dpi = 300)
ggsave("fig_2.1.3.pdf", plot = plot_ev2, width = thesis_width, height = thesis_height, dpi = 300)

cat("--- All plots successfully exported as PDF files for the thesis manuscript ---\n")
