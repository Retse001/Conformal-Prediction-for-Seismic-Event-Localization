...
# --- EVENT DEFINITIONS (3 Events, High Separation) ---
...
# --- PEAK GENERATION WITH PHYSICAL AMPLITUDE CALCULATION ---
vp <- 6.0   
vs <- vp/1.75   
...
for(i in 1:nrow(eventi_verita)) {
  ...
  for(j in 1:nrow(stazioni)) {
    ...
    if(d < 0.1) d <- 0.1 
    # # Arrival times calculation (with Log-Normal to simulate realistic delays)
    tp <- ev$t0 + (d / vp) + rlnorm(1, meanlog = -0.7, sdlog = 0.6)
    ts <- ev$t0 + (d / vs) + rlnorm(1, meanlog = -0.5, sdlog = 0.6) # S solitamente più incerta
    # Physical Amplitude Calculation
    # log_amp = constant + magnitude - attenuation + noise
...
rumore <- data.frame(
  ...
  timestamp = runif(n_rumore, 0, 300), 
...
# ---  SAVING ---
write_csv(stazioni, "1.stazioni_networkAS.csv")
write_csv(picks_finali, "1picks_simulatiAS.csv")
write_csv(eventi_verita, "1eventi_veritaAS.csv")